...
class TrainDataLoader(AbstractDataLoader):
    ...
    def adtpr(self,adj):
        G = nx.Graph() 
        for i in range(0,adj.size(1)):
            G.add_edge(adj[0][i], adj[1][i])
        pagerank = nx.pagerank(G)
        for u, v, data in G.edges(data=True):
            data['weight'] = pagerank[v]
        edge_weights = nx.get_edge_attributes(G, 'weight')
        pr = np.array(list(edge_weights.values()))
        pr = torch.tensor(pr)

        return pr

    def centrality(self,adj):
        G = nx.Graph() 
        temp = []
        for i in range(0,adj.size(1)):
            G.add_edge(adj[0][i], adj[1][i])
        ...
        for edge in G.edges:
            edge_wei = centrality_list[edge[0]] + centrality_list[edge[1]]
            temp.append(edge_wei)
        cr = torch.tensor(temp)  
        return cr
